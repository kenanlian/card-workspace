/* Presentational primitives for the v2 landing. Props in, callbacks out. */
...
/* --- A tabbed index card ------------------------------------------------
   Carries the section's number on its tab, the kicker in the left gutter, and
   the title across both content columns. Children supply `.cw-sec__text` and
   `.cw-sec__media`. */
function SectionCard({ id, index, kicker, title, order = 0, children }) {
  ...
  return (
    ...
    >
      <span className="cw-card__tab">{index}</span>
      <div className="cw-card__grid">
        <p className="cw-label cw-sec__kicker">{kicker}</p>
        <h2 className="cw-h2 cw-sec__title">{title}</h2>
        {children}
      </div>
    </section>
  );
...
function ImageSlot({ id, label, desc, minHeight, ui }) {
  const storageKey = `cw-slot-v2:${id}`;
